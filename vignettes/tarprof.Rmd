...
Once the targets pipeline has finished, we can access the profiling data using [profile_data].
...
We can integrate this information using `profile_per_target`:
...
Finally we can use `memory_plot` to represent the memory usage 
